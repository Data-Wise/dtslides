...
- ~~**RevealJS plugin loading**~~ — ✅ Resolved. Deleted `unm-revealjs.js` (boilerplate Quarto extension template that bound `T` to `deck.shuffle()`). It was never registered in `_extension.yml` (no `revealjs-plugins:` key) and never loaded.
